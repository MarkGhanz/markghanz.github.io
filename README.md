...
## A/B Difference
I found it like its younger brother KP40 has an A/B hardware difference for CN and Intl units. The one on minixpc was BTP-KP70A, same as on JD, initially, and it was changed to KP70-B. Every KP70 on Aliexpress is BTP-KP70B. The "A" uses NearLink and polls at 2000Hz maximum and "B" uses Beitong's proprietary 1000Hz 2.4G while its dongle shows up as `20bc:5160` Shenzhen Shanwan blah blah blah as revealed by `lsusb` on Linux if successfully connected but before claimed by other kernel gamepad drivers, as shown in ![connected](Screenshot_from_2025-12-26_17-07-12.png). The Linux on my machine was previously Arch Linux with kernel 6.18.0, but on NixOS (`nixos-unstable-small`) with kernel 6.18.2, the results are the same. In addition, the only data available on gamepadla showed firmware version 1.1.9. However, the latest firmware for the "B" variant from their true English software was 1.0.3 (initial test, retested on 1.0.4) for both controller and the dongle. So it proved that user on gamepadla imported an "A" variant and nobody online including me knows what are the real sticks latency and button latency for the "B" variant. I cannot afford to tear the controller apart to test it using P82. The dongle itself unconnected had `20dd:5160` as USB VID:PID, as shown by ![Dongle Alone](Screenshot_from_2025-12-26_17-06-19.png). I was not saying either A or B is superior to the other as the AI functionalities on "A" according to bilibili testers were more of gimmicks with quirks. My chat with their customer service didn't reveal any difference besides the wireless transmission and AI.   
...
That's WILDLY DIFFERENT from BTP-KP70B, as shown in ![connected](Screenshot_from_2025-12-26_17-07-12.png).   
...
71Hz. Verified using both gamepadla's polling 1.3.1.4 ![Polling dongle](Screenshot_from_2025-12-26_17-17-48.png) and evtest ![evtest dongle](Screenshot_from_2025-12-26_17-30-55.png).  
...
Its wired performance is also abyssmal. Only 200Hz. Again verified using both gamepadla's polling 1.3.1.4 ![Polling wired](Screenshot_from_2025-12-26_17-25-07.png) and evtest ![evtest wired](Screenshot_from_2025-12-26_17-30-13.png).  
So I am pretty sure that all these behaviors are 100% intentional, intentional malice.
## Conclusion
